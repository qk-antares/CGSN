...
class CGSNGin(nn.Module):
    def __init__(self, args, onehot_dim):
        super(CGSNGin, self).__init__()
        self.args = args
    ...
    def setup_layers(self):
        ...
        gnn_settings = [
            dict(
                nn=torch.nn.Sequential(
                    torch.nn.Linear(gnn_filters[i - 1], gnn_filters[i]),
                    torch.nn.ReLU(),
                    torch.nn.Linear(gnn_filters[i], gnn_filters[i]),
                ),
                train_eps=True,
            )
        ...
        gnn_settings.insert(
            ...
            dict(
                nn=torch.nn.Sequential(
                    torch.nn.Linear(self.onehot_dim, gnn_filters[0]),
                    torch.nn.ReLU(),
                    torch.nn.Linear(gnn_filters[0], gnn_filters[0]),
                ),
                train_eps=True,
            ),
        )

        setattr(self, "gnn{}".format(1), DenseGINConv(**gnn_settings[0]))
        for i in range(1, self.gnn_numbers):
            setattr(self, "gnn{}".format(i + 1), DenseGINConv(**gnn_settings[i]))

        # affinity模块
        self.affinity = Affinity(d=gnn_filters[-2])
        # sinkhorn模块
        self.sinkhorn = Sinkhorn(
            max_iter=self.args.max_iter, tau=self.args.tau, epsilon=self.args.epsilon
        )
        # cross_graph模块
        ...
        # 图图交互的ntn模块
        self.tensor_network = DenseTensorNetwork(
    ...
    def forward_att_feat_agg_layers(self, emb1, adj1, mask1, emb2, adj2, mask2):
        for i in range(1, self.gnn_numbers + 1):
            ...
            emb1, emb2 = (
                gnn_layer(x=emb1, adj=adj1, mask=mask1, add_loop=True),
            ...
            if i == self.gnn_numbers - 1:
                ...
                s = self.sinkhorn(
                    s, torch.sum(mask1, dim=1), torch.sum(mask2, dim=1), dummy_row=True
                )

                new_emb1 = self.cross_graph(
    ...
    def forward(
        ...
        :return:
        """
        emb1, emb2 = self.forward_att_feat_agg_layers(
            batch_feat_1,
            batch_adj_1,
            batch_mask_1,
            batch_feat_2,
